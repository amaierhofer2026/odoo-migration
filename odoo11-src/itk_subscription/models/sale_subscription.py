...
class SaleSubscription(models.Model):
    ...
    pricelist_id = fields.Many2one('product.pricelist', string='Pricelist', required=True, track_visibility='onchange')
    currency_id = fields.Many2one('res.currency', related='pricelist_id.currency_id', string='Currency', readonly=True, track_visibility='onchange')
    ...
    minimum_contract_period_unit = fields.Selection([('daily', 'Day(s)'), ('weekly', 'Week(s)'),
                                                    ...
                                                    # string='Unit',
                                                    # help="",
                                                    # default='yearly', track_visibility='onchange'
                                                    store=True,
                                                    readonly=True,
                                                    )
    ...
    sale_order_confirmation_date = fields.Date(string='Date of first Saleorder',  # default=fields.Date.today,
                                               help="")
    ...
    def _compute_ctp_details(self):
        # slctn = self.template_id.minimum_contract_life_unit
    ...
    def action_open_sales(self):
        ...
        return {
            ...
            "views": [[self.env.ref('itk_subscription.sale_order_view_tree_subscription').id, "tree"],
    ...
    def _prepare_invoice_data(self):
        ...
        return {
            ...
            'comment': ("Diese Rechung umfasst folgenden Zeitraum: %s - %s") % (
...
class SaleSubscriptionLine(models.Model):
    ...
    def action_set_partner_id_of_line(self):
        # Action called by function call in xml-import for setting partner_id, salesperson_id and  of a subscription.line
    ...
    #     return self.analytic_account_id.partner_id.id

    # def set_line_partner_id(self, line):
    #     # added for setting partner_id
    #     line_sudo = line.sudo()
    #     line_sudo.partner_id = self.analytic_account_id.partner_id.id
    #     p = self.analytic_account_id.partner_id
    #     a = p.id
    #     line.partner_id = p
    #     # x = self.analytic_account_id.partner_id
    #     # y = x.id
    #     # self.partner_id = y
    #     6 + 6
    ...
    def _compute_price_subtotal(self):
        ...
        # # added for setting partner_id
        # x =
        # y = self.partner_id = x.id
        # #            6 + 6

        for line in self:

            # self.set_line_partner_id(line)
            line_sudo = line.sudo()
            # x = line_sudo.analytic_account_id.pricelist_id
            # y = line.price_unit
...
class SaleSubscriptionTemplate(models.Model):
    ...
    minimum_contract_life = fields.Integer('Min. Contract Life', size=2)
    minimum_contract_life_unit = fields.Selection([('daily', 'Day(s)'), ('weekly', 'Week(s)'),
                                                   ('monthly', 'Month(s)'), ('yearly', 'Year(s)'), ],
                                                  string='Unit',
                                                  help="",
                                                  default='yearly', track_visibility='onchange')
    contract_termination_period_number = fields.Integer('Notice Period', size=2)
    contract_termination_period_unit = fields.Selection([('daily', 'Day(s)'), ('weekly', 'Week(s)'),
                                                         ('monthly', 'Month(s)'), ('yearly', 'Year(s)'), ],
                                                        string='Unit',
                                                        help="",
