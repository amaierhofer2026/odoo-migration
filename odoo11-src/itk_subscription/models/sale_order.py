...
class SaleOrder(models.Model):
    ...
    def action_open_subscriptions(self):
        ...
        action = self.env.ref('itk_subscription.sale_subscription_action').read()[0]
        if len(subscriptions) > 1:
            action['domain'] = [('id', 'in', subscriptions.ids)]
        elif len(subscriptions) == 1:
            action['views'] = [(self.env.ref('itk_subscription.sale_subscription_view_form').id, 'form')]
    ...
    def _prepare_subscription_data(self, template):
        ...
        values = {
            ...
            'sale_order_confirmation_date': self.confirmation_date,
            'name': template.name,
            'state': 'open',
...
class SaleOrderLine(models.Model):
    ...
    def _prepare_subscription_line_data(self):
        ...
        # self.ensure_one()
        # part_id = self.order_partner_id.id
        values = list()
        for line in self:
            partner_id = line.order_partner_id.id
            qty_multiplication_factor = line.order_partner_id.multi_factor
            salesperson_id = line.order_id.user_id.id,
            values.append((0, False, {
                #  start insert the partner_id to transfer to subscription_line
                'partner_id': partner_id,
                #  end insert the partner_id to transfer to subscription_line
                #  start insert the user_id to transfer to subscription_line
                'salesperson_id':salesperson_id,
                #  end insert the partner_id to transfer to subscription_line
                #  start insert the analytic_account_id to transfer to subscription_line
                # 'analytic_account_id': analytic_account_id,
                #  end insert the analytic_account_id to transfer to subscription_line
                'product_id': line.product_id.id,
                'name': line.name,
                # add value for ewz multifactor
                'qty_multiplication_factor': qty_multiplication_factor,
                # end
