...
class CustomerPortal(CustomerPortal):
    ...
    def my_subscription(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        ...
        return request.render("itk_subscription.portal_my_subscriptions", values)
...
class sale_subscription(http.Controller):
    ...
    def subscription(self, account_id, uuid='', message='', message_class='', **kw):
        ...
        providers = list(request.env['payment.provider'].search([
        ...
        dummy, action = request.env['ir.model.data'].get_object_reference('itk_subscription', 'sale_subscription_action')
        values = {
            'account': account,
        ...
        return request.render("itk_subscription.subscription", values)
    ...
    def payment(self, account_id, uuid=None, **kw):
        ...
        if payment_token:
            ...
            tx = account.sudo()._do_payment(payment_token, new_invoice)
            if tx.html_3ds:
                return tx.html_3ds
    ...
    def payment_accept(self, sub_uuid, tx_id, **kw):
        ...
        tx._handle_notification_data('', kw)
