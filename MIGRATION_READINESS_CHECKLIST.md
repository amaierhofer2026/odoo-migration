...
- **EUR-Symbol in `res_currency.symbol`:** auf der **VM** seit Session 81 korrekt `€`; auf der **lokalen** Instanz weiterhin Mojibake `Ôé¼` (verifiziert 10.09.2026, Session 83) — Sichtprüfung lokal offen. ⚠️
...
| Währungssymbol | res_currency.symbol (EUR) | **VM: `€` (korrekt seit Session 81)** — **lokal: `Ôé¼` (Mojibake)** | **TEILWEISE BEHOBEN** | CP850-Mojibake in der lokalen DB noch vorhanden (F1) | Symbol lokal korrigieren (nach Freigabe) — `scripts/fix_ui_encoding_de.py` ist auf die VM-URL hart kodiert | ja (DB, 10.09.2026) |
...
| Zeitzone | **2 von 14 aktiven Benutzern** mit `Europe/Vienna` (uid 2, 8); **12 aktive Benutzer ohne** → UTC-Fallback (Stand 10.09.2026, Session 83) | **ANPASSUNG NÖTIG** | Zeitzone fehlt bei 12 aktiven Benutzern | Zeitzone Europe/Vienna für aktive Benutzer setzen (nach Freigabe) | ja (DB, 10.09.2026) |
...
|---|---|---|---|---|
| F1 | C/Währung | EUR-Symbol in `res_currency.symbol` war `Ôé¼` statt `€` (CP850-Mojibake); weitere Symbole betroffen (nicht installiert/inaktiv) | VM-DB res_currency id 126; Vergleich VM/lokal 10.09. (Session 83) | **BEHOBEN auf der VM** (02.09., Session 81: symbol = `€`); **LOKAL OFFEN** — lokale DB hat weiterhin `Ôé¼` (verifiziert 10.09.2026) |
...
| F8 | D/Basiseinstellungen | Zeitzone nur bei **2 von 14 aktiven** Benutzern gesetzt (Europe/Vienna: uid 2, 8); **12 aktive Benutzer ohne tz** (UTC-Fallback) — Stand 10.09.2026 (Session 83); beruflich 18 Benutzer gesamt, davon 14 aktiv | res_users/res_partner | ANPASSUNG NÖTIG |
...
- **Offen:** Browser-Sichtprüfung der korrigierten UI (Chrome-„Allow remote debugging“-Popup nötig, wie in früheren Sessions), Deploy der Code-Commits auf die VM (SSH), de.po-Nachladen funktioniert in dieser Umgebung NICHT (msgstr-Gerüste leer bzw. Import wirkungslos → dokumentierter Weg sind die Fix-Skripte).
...
- **VM-Zugang (Befund 10.09.2026, Session 83):** Port 22 der Test-VM ist VM-seitig für das **gesamte Internet** gefiltert (6/6 externe Nodes Timeout; 443 von denselben Nodes offen; alternative SSH-Ports ebenfalls zu). Code-Deploy (`git pull` in `/opt/odoo18`) und Modul-Upgrades auf der VM sind damit **angehalten** — read-only-Analysen und DB-Fixes laufen über HTTPS-JSON-RPC. Die **lokale** Instanz hat zudem **nicht** den Fix-Stand der VM (EUR-Symbol F1, de_DE-Slots aus Session 81) — lokale Abweichungen werden getrennt ausgewiesen.
