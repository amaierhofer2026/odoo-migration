...
| F33 | Datenvollstaendigkeit (Dateien) | **Filestore unvollstaendig:** Anhaenge/Bilder, die in der DB referenziert sind, fehlen als Datei im Filestore. Auf der VM innerhalb von 48 h **74 verschiedene fehlende Dateien** (`/var/lib/odoo/filestore/odoo18_test/...`), u. a. Website-Logo/Favicon und Partner-Avatare -> die zugehoerigen `/web/image/...`-Anfragen enden mit 500. Betrifft **beide** Instanzen (lokaler Filestore enthaelt nur 29 Dateien, VM 50 MB) | VM-Log 48 h (210 `FileNotFoundError`), Verzeichnisvergleich `filestore/` lokal und in beiden Containern (14.09.2026, Session 88) | **Hinweis / offen** - vorbestehend, **nicht** durch diese Session verursacht; Klaerung noetig, woher die vollstaendigen Dateien kommen (Backup/Original-Server) |
